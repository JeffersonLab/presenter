...
CREATE SEQUENCE PRESENTER_OWNER.TEAM_ID
    START WITH 1
    NOCYCLE
    NOCACHE
    ORDER;

CREATE SEQUENCE PRESENTER_OWNER.TEAM_STATUS_REPORT_ID
    START WITH 1
    NOCYCLE
    NOCACHE
    ORDER;
...
(
    ...
    PD_PRESENTATION_TYPE VARCHAR2(16) NOT NULL  CONSTRAINT  PD_PRESENTATION_CK2 CHECK (PD_PRESENTATION_TYPE IN ('RUN', 'SAM', 'LSD', 'SUM1', 'SUM2', 'SUM3', 'HCO', 'PD')),
...
CREATE TABLE PRESENTER_OWNER.TEAM
(
    TEAM_ID              INTEGER NOT NULL,
    NAME                 VARCHAR2(128 CHAR) NOT NULL,
    DESCRIPTION          VARCHAR2(1024 CHAR) NULL,
    DIRECTORY_GROUP_NAME VARCHAR2(64 CHAR) NOT NULL,
    WEIGHT               INTEGER DEFAULT 0 NOT NULL,
    ARCHIVED_YN          VARCHAR2(1 CHAR) DEFAULT 'N' NOT NULL,
    CONSTRAINT TEAM_PK PRIMARY KEY (TEAM_ID),
    CONSTRAINT TEAM_AK1 UNIQUE (NAME),
    CONSTRAINT TEAM_CK1 CHECK (ARCHIVED_YN IN ('Y', 'N'))
);

CREATE TABLE PRESENTER_OWNER.TEAM_STATUS_REPORT
(
    TEAM_STATUS_REPORT_ID INTEGER NOT NULL,
    TEAM_ID               INTEGER NOT NULL,
    YMD                   TIMESTAMP(0) NOT NULL,
    ACCOMPLISHED          CLOB NULL,
    IN_PROGRESS           CLOB NULL,
    PLANNED               CLOB NULL,
    ROADBLOCKS            CLOB NULL,
    CONSTRAINT TEAM_STATUS_REPORT_PK PRIMARY KEY (TEAM_STATUS_REPORT_ID),
    CONSTRAINT TEAM_STATUS_REPORT_FK FOREIGN KEY (TEAM_ID) REFERENCES PRESENTER_OWNER.TEAM(TEAM_ID),
    CONSTRAINT TEAM_STATUS_REPORT_AK1 UNIQUE (TEAM_ID, YMD),
    CONSTRAINT TEAM_STATUS_REPORT_CK1 CHECK (EXTRACT(HOUR FROM YMD) = 0 AND EXTRACT(MINUTE FROM YMD) = 0 AND EXTRACT(SECOND FROM YMD) = 0)
);

CREATE TABLE PRESENTER_OWNER.SETTING
(
