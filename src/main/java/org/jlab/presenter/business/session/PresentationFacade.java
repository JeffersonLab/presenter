...
public class PresentationFacade extends AbstractFacade<Presentation> {
  ...
  @EJB PresentationLogFacade presentationLogFacade;
  ...
  @PermitAll
  @TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
  public String checkAuthorized(PresentationType type) throws EJBAccessException {
    String[] response = getUnauthorizedReason(type);
    ...
    if (presentation != null && presentation.getSlideList() != null) {
      ...
      for (Slide slide : presentation.getSlideList()) {
        if (slide instanceof IFrameSlide) {
          ...
          imageSlideHtml =
              imageSlideHtml.replaceFirst(
                  "<div class=\"img-div(.*?)</div>", "[figure:" + figureNum++ + "]");
  ...
  public void createLogIdRecord(String username, BigInteger presentationId, long logId) {
    PresentationLogPK pk =
        new PresentationLogPK(presentationId, BigInteger.valueOf(logId), username);
  ...
  public long sendELog(
    ...
    if (presentationType != PresentationType.PD_PRESENTATION
        && entrymaker != null
        && !entrymaker.isEmpty()
        && !entrymaker.equalsIgnoreCase("ANONYMOUS")) {
      username = entrymaker;
    }
    ...
    // REMEMBER: This field is ignored and the actual submitter is used instead if the actual
    // submitter isn't a special user like "glassfish" or "wildfly".  This means on local testing
    // this won't work!
  ...
  private String findPDUsername() {
    ...
    Query q =
        em.createNativeQuery(
            "select username from presenter_owner.shift_plans, presenter_owner.staff where program_deputy = staff_id and begins_at between (sysdate - 0.3) and (sysdate + 0.3)");
  ...
  @PermitAll
  public long publicSendELogRequest(
      BigInteger presentationId, HttpServletRequest request, HttpServletResponse response)
  ...
  @PermitAll
  public long sendELog(
      Presentation presentation, HttpServletRequest request, HttpServletResponse response)
      throws WebAppException {
    try {
      List<String> imageData = new ArrayList<>();
      String body =
          getPresentationHTML(request, response, presentation.getPresentationId(), imageData);
      ...
      switch (presentationType) {
        case PD_PRESENTATION:
          logId = PresentationMenuUtil.logPd((PDPresentation) presentation, this, body, imageData);
          break;
        case CC_PRESENTATION:
          logId = PresentationMenuUtil.logCc((CCPresentation) presentation, this, body, imageData);
          break;
        case LO_PRESENTATION:
          logId = PresentationMenuUtil.logLo((LOPresentation) presentation, this, body, imageData);
          break;
        case LASO_PRESENTATION:
          logId =
              PresentationMenuUtil.logLaso((LASOPresentation) presentation, this, body, imageData);
          break;
        case UITF_PRESENTATION:
          logId =
              PresentationMenuUtil.logUitf((UITFPresentation) presentation, this, body, imageData);
          break;
        default:
  ...
  public void merge(Presentation presentation, List<Slide> fromList, boolean newSlidesGoFirst) {
    ...
    for (Slide s : toList) {
      if (s.getSyncFromSlideId() != null) {
        ...
        if (duplicate != null) {
          LOGGER.log(Level.WARNING, "Duplicate in toList; SlideId: {0}", duplicate.getSlideId());
          toDuplicates.add(s);
        }
      }
    }
    ...
    // Comment out filtering duplicates due to the fact that we now allow new non-synced slides
    // which means slideId = null (physics summary graph iframe and fsd graph both are null slideId)
    ...
    for (int i = 0; i < fromList.size(); i++) {
      ...
      if (updatable.containsKey(s.getSlideId())) {
        ...
        n.setOrderId(
            Long.valueOf(
                i)); /*We keep the input order number so we can possibly use it later to determine where this new slide goes; it is overwritten later anyways*/
    ...
    //
    // - or if not from an existing synced presentation (or not a synced slide) then try to put them
    ...
    for (Slide s : newSlides) {
      // System.err.println("new slide original order: " + s.getOrderId() + "; type: " +
      // s.getSlideType());

      boolean added = false;
      if (s.getSyncFromSlideId() != null) {
        // If slide comes from synced presentation then try to find the last slide already included
        // from that presentation and append there
      ...
      if (!added) { // Either wasn't syncable or didn't have slide from same presentation
        // so try to insert in order given by input list relative to closest predeceeding "anchor"
        // synced slide
        // System.err.println("doing the harder thing...");
        Long originalIndex =
            s.getOrderId(); // We are using input order to help determine order in merged slide set.
          ...
          for (int i = (originalIndex.intValue()) - 1; i >= 0; i--) {
            ...
            if (sId != null) {
              ...
              for (int j = 0; j < toList.size(); j++) {
                ...
                if (e.getSyncFromSlideId() != null && e.getSyncFromSlideId().equals(sId)) {
      ...
      if (!added) { // couldn't add slide behind last slide from same presentation / or it isn't a
        // synced slide and doesn't have preceeding "anchor" slides to be positioned
        // relative to - so just add it to list which will be added to beginning
        // System.err.println("adding to the end: " + (toList.size()));
        unplacedList.add(s);
      }
    }

    // Add unplaced list to beginning of toList by adding toList to end of unplacedList and
      ...
      for (Slide s : slideList) {
        if (s.getOrderId() != order) {
          LOGGER.log(
              ...
              new Object[] {
                presentation.getPresentationId(), s.getSlideId(), order, s.getOrderId()
              });
          s.setOrderId(order);
  ...
  @PermitAll
  public void insertDailyGraphs(
      PDPresentation presentation, DailySlideGenerator[] dailySlideGeneratorArray) {
    List<Slide> slideList = presentation.getSlideList();
        ...
        if (addDaily) {
          if (dailySlideGeneratorArray != null) {
            for (int j = 0; j < dailySlideGeneratorArray.length; j++) {
              Slide dailySlide = dailySlideGeneratorArray[j].getSlideForDay(day);
    ...
    if (presentation != null) {
      ...
      switch (presentationType) {
        case PD_PRESENTATION:
          ...
          description =
              ShowInfo.getPdShowName(
                  pdPresentation.getPdPresentationType(), pdPresentation.getDeliveryYmd());
          break;
        case CC_PRESENTATION:
          CCPresentation ccPresentation = (CCPresentation) presentation;
          description = ShowInfo.getCcShowName(ccPresentation.getYmd(), ccPresentation.getShift());
          break;
        case LASO_PRESENTATION:
          LASOPresentation lasoPresentation = (LASOPresentation) presentation;
          description =
              ShowInfo.getLasoShowName(lasoPresentation.getYmd(), lasoPresentation.getShift());
          break;
        case LO_PRESENTATION:
  ...
  public class AuditRecord {
    ...
    public AuditRecord(
        BigInteger presentationId, String user, Long lastModifiedMillis, String description) {
