 ...
 * @author ryans
 */
@WebServlet(name = "LOMenu", urlPatterns = {"/lo-menu"})
public class LOMenu extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request,
        ...
        getServletConfig().getServletContext().getRequestDispatcher(
                "/WEB-INF/views/lo-menu.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if (messages.isEmpty()) {
            presentationId = loPresentationFacade.findIdByYmdAndShift(ymd,
                    shift);
    ...
    public List<Slide> getNewShiftLogSlides(HttpServletRequest request,
            HttpServletResponse response, LOPresentation presentation)
            throws ServletException, IOException {
        ArrayList<Slide> slides = new ArrayList<>();

        // Slide 1
        ShiftInfoSlide shiftSlide = new ShiftInfoSlide(presentation.getYmd(),
                presentation.getShift(), ShiftSlideType.LO);
