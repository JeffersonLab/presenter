...
@WebServlet(
    name = "SendToElog",
    urlPatterns = {"/send-to-elog"})
public class SendToElog extends HttpServlet {

  private static final Logger logger = Logger.getLogger(SendToElog.class.getName());
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (errorReason == null) {
      try {
        ...
        logger.log(Level.FINE, "Sending to elog; " + "Presentation ID: {0}", presentationId);
    ...
    if (error) {
      logger.log(Level.SEVERE, "PrintWriter Error in " + "SendToElog.doPost");
    }
  }
}
