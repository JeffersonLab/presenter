...
/**
 * Servlet controller for proxying requests to Image Magic Resize service even for external / public
 * users.
 *
...
public class Resize extends HttpServlet {
  ...
  private static final Logger LOGGER = Logger.getLogger(Resize.class.getName());
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    // All of this proxy complexity is really just to ensure only authenticated users are using the
    // resize service.
    ...
    /**
     * Amazingly, sending multipart (binary data) form post is not supported in Java 11 or Java EE
     * 8. Looks like maybe supported in Jakarta EE 10, which just came out a few months ago
     * (September 2022). Uploading a binary file on the web was defined in 1998
     * (https://www.rfc-editor.org/rfc/rfc2388).
     *
     * <p>Note: receiving multipart data IS supported. Though confusingly getParameter() is
     * overloaded to access parts that are Strings vs getPart() for binary.
     *
     * <p>https://github.com/jakartaee/rest/issues/418
     *
     * <p>Apparently gold standard is still https://hc.apache.org/
     *
     * <p>We need to POST with multipart/form-data instead of application/x-www-form-urlencoded
     */
    ...
    if (allowStretching != null) {
      builder.addTextBody("allow-stretching", allowStretching);
    }
    if (ignoreAspectRatio != null) {
      builder.addTextBody("ignore-aspect-ratio", ignoreAspectRatio);
    }
    if (outputDataUri != null) {
      builder.addTextBody("output-data-uri", outputDataUri);
    }
    if (forcePng != null) {
      builder.addTextBody("force-png", forcePng);
    }
    ...
    try {
      HttpResponse<InputStream> resizeResponse =
          client.send(resizeRequest, HttpResponse.BodyHandlers.ofInputStream());
