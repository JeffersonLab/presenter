...
@WebServlet(
    name = "SaveSlide",
    urlPatterns = {"/save-slide"})
public class SaveSlide extends HttpServlet {

  private static final Logger logger = Logger.getLogger(SaveSlide.class.getName());
  @EJB TitleBodySlideFacade titleBodySlideFacade;
  @EJB BodySlideFacade bodySlideFacade;
  @EJB ImageSlideFacade imageSlideFacade;
  @EJB TitleImageSlideFacade titleImageSlideFacade;
  @EJB TitleBodyImageSlideFacade titleBodyImageSlideFacade;
  @EJB IFrameSlideFacade iframeSlideFacade;
  @EJB ShiftInfoSlideFacade shiftInfoSlideFacade;
  @EJB BtaShiftInfoSlideFacade btaShiftInfoSlideFacade;
  @EJB PdInfoSlideFacade pdInfoSlideFacade;
  @EJB PdBeamAccSlideFacade pdBeamAccSlideFacade;
  @EJB PdAccessSlideFacade pdAccessSlideFacade;
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      slideId = ConvertAndValidateUtil.convertAndValidateBigIntegerParam(request, "slideId");
    ...
    try {
      type = ConvertAndValidateUtil.convertAndValidateSlideType(request, "slideType");

      if (type == null) {
        errorReason = "Parameter 'slideType' " + "must not be null";
      }
    } catch (Exception e) {
      ...
      try {
        switch (type) {
          ...
          case PD_ACCESS_SLIDE:
            ...
            break;
          default:
            throw new IllegalArgumentException("Unknown SlideType: " + type);
        }
        ...
        if (t instanceof ConstraintViolationException) {
          ...
          for (ConstraintViolation<?> violation : cve.getConstraintViolations()) {
            errorReason =
                errorReason + violation.getPropertyPath() + " " + violation.getMessage() + ". ";
          }
        } else {
    ...
    if (errorReason == null) {
      ...
      if (audit != null) {
        logger.log(
            Level.FINEST, "SAVE SLIDE - Last modified is {0}", audit.getLastModifiedMillis());
        xml =
            xml
                + "<span class=\"presentation\" data-last-modified-millis=\""
