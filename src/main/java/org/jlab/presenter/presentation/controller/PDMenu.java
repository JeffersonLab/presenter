...
public class PDMenu extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    List<PDPresentationType> typeList = new ArrayList<>(Arrays.asList(PDPresentationType.values()));
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      type = ConvertAndValidateUtil.convertAndValidatePDType(request, "type");
      ...
      switch (type) {
        ...
        case PD:
          if (dayOfWeek == Calendar.SATURDAY || dayOfWeek == Calendar.SUNDAY) {
            messages.put("date", "Selected type can only be delivered Monday-Friday");
          }
          break;
        case SUM1:
        case SUM2:
        case SUM3:
          if (dayOfWeek != Calendar.WEDNESDAY) {
            messages.put("date", "Selected type can only be delivered Wednesday");
          }
          break;
      }
    }
    ...
    if (messages.isEmpty()) {
      presentation = pdPresentationFacade.findByYmdAndPDType(ymd, type);
      ...
      if (presentationId == null
          && !presentationFacade.isAuthorized(PresentationType.PD_PRESENTATION)) {
        messages.put(
            "error", "No presentation found for specified criteria.  Login as PD to create.");
      }
    }

    if (!messages.isEmpty()) {
      List<PDPresentationType> typeList =
          new ArrayList<>(Arrays.asList(PDPresentationType.values()));
  ...
  private List<Slide> getNewLSDSlides(
      HttpServletRequest request, HttpServletResponse response, PDPresentation presentation)
      throws ServletException, IOException {
  ...
  private List<Slide> getNewHCOSlides(
      HttpServletRequest request, HttpServletResponse response, PDPresentation presentation)
      throws ServletException, IOException {
    ArrayList<Slide> slides = new ArrayList<>();
  ...
  private List<Slide> getNewPDSlides(
      HttpServletRequest request, HttpServletResponse response, PDPresentation presentation)
      throws ServletException, IOException {
    ArrayList<Slide> slides = new ArrayList<>();
  ...
  private List<Slide> getNewRunSlides(
      HttpServletRequest request, HttpServletResponse response, PDPresentation presentation) {
    ArrayList<Slide> slides = new ArrayList<>();
  ...
  private List<Slide> getNewSADSlides(
      HttpServletRequest request, HttpServletResponse response, PDPresentation presentation)
      throws ServletException, IOException {
    ArrayList<Slide> slides = new ArrayList<>();
  ...
  private List<Slide> getNewSUM1Slides(
      HttpServletRequest request, HttpServletResponse response, PDPresentation presentation)
      throws ServletException, IOException {
    ArrayList<Slide> slides = new ArrayList<>();
  ...
  private List<Slide> getNewSUM2Slides(
      HttpServletRequest request, HttpServletResponse response, PDPresentation presentation)
      throws ServletException, IOException {
    ArrayList<Slide> slides = new ArrayList<>();
    ...
    PdInfoSlide previousGoalSlide =
        pdPresentationFacade.findFirstGoalSlide(
            TimeUtil.getOneWeekBefore(presentation.getDeliveryYmd()));

    if (previousGoalSlide != null) {
      sum2P1Slide.setPd(previousGoalSlide.getPd());
      sum2P1Slide.setBody(
          previousGoalSlide.getBody().replaceAll("\\(upcoming week\\)", "(previous week)"));
    }
  ...
  private List<Slide> getNewSUM3Slides(
      HttpServletRequest request, HttpServletResponse response, PDPresentation presentation)
      throws ServletException, IOException {
    ArrayList<Slide> slides = new ArrayList<>();
    ...
    // Check if goals were set last week and copy them over if found
    BodySlide previousGoalSlide =
        pdPresentationFacade.findSecondGoalSlide(
            TimeUtil.getOneWeekBefore(presentation.getDeliveryYmd()));
    ...
    PdInfoSlide info =
        pdPresentationFacade.findFirstGoalSlide(
            TimeUtil.getTwoWeekBefore(presentation.getDeliveryYmd()));
    ...
    List<Slide> incomingSlides = pdPresentationFacade.getIncomingSlides(presentation, order, true);
