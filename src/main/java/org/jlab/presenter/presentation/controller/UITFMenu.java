 ...
 * @author ryans
 */
@WebServlet(name = "UITFMenu", urlPatterns = {"/uitf-menu"})
public class UITFMenu extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request,
        ...
        getServletConfig().getServletContext().getRequestDispatcher(
                "/WEB-INF/views/uitf-menu.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if (messages.isEmpty()) {
            presentationId = uitfPresentationFacade.findIdByYmdAndShift(ymd,
                    shift);
    ...
    public List<Slide> getNewShiftLogSlides(HttpServletRequest request,
            HttpServletResponse response, UITFPresentation presentation)
            throws ServletException, IOException {
        ArrayList<Slide> slides = new ArrayList<>();

        // Slide 1
        ShiftInfoSlide shiftSlide = new ShiftInfoSlide(presentation.getYmd(),
                presentation.getShift(), ShiftSlideType.UITF);
