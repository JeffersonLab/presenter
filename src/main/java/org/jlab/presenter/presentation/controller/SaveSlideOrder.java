...
@WebServlet(
    name = "SaveSlideOrder",
    urlPatterns = {"/save-slide-order"})
public class SaveSlideOrder extends HttpServlet {

  private static final Logger logger = Logger.getLogger(SaveSlideOrder.class.getName());
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (errorReason == null) {
      try {
        ...
        Presentation presentation = presentationFacade.findWithSlides(presentationId);
        ...
        List<Slide> reorderedSlides = convertAndValidateReorderedSlideList(slideIds, presentation);
        ...
        logger.log(
            Level.FINE,
            "Saved presentation slide order; " + "Presentation ID: {0}",
            presentationId);
    ...
    if (errorReason == null) {
      ...
      if (audit != null) {
        logger.log(
            Level.FINEST,
            "UPDATE SLIDE ORDER - Last modified is {0}",
    ...
    if (error) {
      logger.log(Level.SEVERE, "PrintWriter Error in " + "SaveSlideOrder.doPost");
    }
  }

  private List<BigInteger> convertAndValidateSlideOrder(HttpServletRequest request) {
    ArrayList<BigInteger> slideIds = new ArrayList<BigInteger>();
    int numSlides = Integer.valueOf(request.getParameter("numSlides"));

    for (int i = 0; i < numSlides; i++) {
      BigInteger id =
          ConvertAndValidateUtil.convertAndValidateBigIntegerParam(request, "slide[" + i + "]");
      if (id == null) {
        throw new IllegalArgumentException("Parameter 'slide[" + i + "]' must not be null");
      }
      slideIds.add(id);
    }
  ...
  private List<Slide> convertAndValidateReorderedSlideList(
    ...
    if (slideIds.size() != slides.size()) {
      throw new IllegalArgumentException("Submitted slide count doesn't match database");
    }
    ...
    for (BigInteger slideId : slideIds) {
      ...
      if (slide == null) {
        throw new IllegalArgumentException(
            "Slide ID not associated with this presentation: " + slideId);
      }
