...
@WebServlet(
    name = "DeleteSlide",
    urlPatterns = {"/delete-slide"})
public class DeleteSlide extends HttpServlet {

  private static final Logger LOGGER = Logger.getLogger(DeleteSlide.class.getName());
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (errorReason == null) {
      try {
        BigInteger slideId =
            ConvertAndValidateUtil.convertAndValidateBigIntegerParam(request, "slideId");
    ...
    if (errorReason == null) {
      ...
      if (audit != null) {
        LOGGER.log(
            Level.FINEST, "DELETE SLIDE - Last modified is {0}", audit.getLastModifiedMillis());
        xml =
            xml
