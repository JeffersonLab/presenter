...
/**
 * A WebFilter that prompts users to authenticate if offsite to view content, but not if onsite or
 * on localhost.
 *
 * @author ryans
 */
@WebFilter(
    filterName = "IPAuthFilter",
    urlPatterns = {"/*"},
    dispatcherTypes = {DispatcherType.REQUEST, DispatcherType.FORWARD},
    asyncSupported = false)
public class IPAuthFilter implements Filter {

  private static final Logger LOGGER = Logger.getLogger(IPAuthFilter.class.getName());

  public static final String[] IGNORE_PATTERNS =
      new String[] {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (ignoreMatch) {
      ...
      LOGGER.log(
          Level.FINEST,
          "Username: {0}, ONSITE_WHITELIST_PATTERN: {1}, ONSITE_WHITELIST_LOCAL: {2}",
          new Object[] {username, patternStr, whitelistlocal});
      ...
      if (redirect) {
        ...
        String redirectUrl =
            absHostUrl
                + contextPath
                + "/sso?returnUrl="
                + URLEncoder.encode(domainRelativeReturnUrl, StandardCharsets.UTF_8);
  ...
  public boolean inWhitelist(String patternStr, String ip) {
    ...
    LOGGER.log(
        Level.FINEST,
        "pattern = {0}, ip = {1}, match = {2}",
        new Object[] {patternStr, ip, matches});
